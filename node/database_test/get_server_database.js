...
app.get('/api/id/:id', function(req,res){
	...
	//Si los parametros son vacios se envia 404 y termina la respuesta
	if (user_id=='[]'){
		res.status(404);
		res.end();
		return;
...
*/
	try {
		...
		connection.query({sql:'SELECT nombre,numero_telefono FROM usuarios WHERE id_usuario= ?',timeout:30000},[user_id],function(err,results){
			...
			//console.log({results});
			console.log("Tiempo transcurrido antes de los if",preif[1]/1000000.);
			//console.log("Tiempo transcurrido antes de los if (otro formato) : %ds %dms", preif[0], preif[1]/1000000);
			
			console.log("Tiempo transcurrido después de los if",postif[1]/1000000.);
